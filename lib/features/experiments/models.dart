...
enum ExperimentDomain { biological, animal, custom }
...
class ExperimentTemplate {
  const ExperimentTemplate(
      ...
      required this.color,
      required this.steps,
      this.domain = ExperimentDomain.biological,
      this.summary = ''});
  final String id;
  final String name;
  final Color color;
  final List<TemplateStep> steps;
  final ExperimentDomain domain;
...
ExperimentTemplate cck8Template() => const ExperimentTemplate(
      ...
      steps: [
        TemplateStep(
            id: 'observe',
            title: '细胞状态观察',
            description: '记录细胞状态与融合度',
            stepType: StepType.observation,
            timeMode: TimeMode.absolute,
            plannedTime: TimeOfDay(hour: 8, minute: 30)),
        TemplateStep(
            id: 'treat',
            title: '加药',
            description: '完成药物处理或换液',
        ...
        TemplateStep(
            id: 'read',
            title: '酶标仪读数',
            description: '记录 OD450 读数',
            stepType: StepType.instrument,
            timeMode: TimeMode.relative,
            offsetMinutes: 120,
            dependsOnId: 'reagent'),
        TemplateStep(
